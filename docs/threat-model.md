...
| T04 | Modified client submits arbitrary content disguised as a PDF | Likely for an abuser | High | Worker requires a PDF header, 60 KB maximum, valid ticket, matching hash, and one-time admission | Hash agreement proves only equality with attacker-declared bytes. Worker does not parse pages or prove use of SealProof's generator. Define a safe production abuse/content-authenticity strategy without exceeding CPU limits | Open — launch gate |
| T05 | Human or bot uses SealProof to send spam, phishing PDFs, or harassing messages | Plausible | Critical | Turnstile, request limits, short ticket, maximum three attempts per role, separate provider idempotency | Turnstile is not rate limiting and a human can pass it. Add Cloudflare/provider rate controls, quotas, monitoring, abuse response, and bounded recipient/domain policies before public launch | Open — launch gate |
