...
| `DONE` | Continue delivery updates for unresolved recipients | Recipient-based progressive polling continues after a mixed outcome on both release and closeout screens; hidden tabs pause, expiry stops reads, and manual refresh remains available. [Local polling evidence](evidence/2026-09-12-progressive-status-polling.md) |
...
| `LATER` | Evaluate a hibernating WebSocket status relay | Reconsider if measured status-request/D1 usage threatens free-tier headroom or update latency causes problems. Compare against the selected progressive polling; require reviewed authorization, reconnect/reconciliation, connection limits, expiry and cleanup before any architecture change |
