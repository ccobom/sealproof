...
| Provider attachment ticket | SEALPROOF | Authorizes one role-specific provider retrieval | Resend receives the URL during submission | Exact ticket is stored only inside a second AES-GCM, attempt-bound D1 envelope | Until closeout or two-hour expiry; URL metadata may remain in provider or infrastructure logs | Deleted with temporary delivery attempts | D1 contains no usable plaintext provider bearer |
